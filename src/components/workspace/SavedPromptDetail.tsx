import { ArrowLeft, Copy, Download, Files, RefreshCcw, Ruler, Trash2 } from 'lucide-react'
import { useState } from 'react'
import type { PromptCollectionRecord, SavedPromptRecord } from '../../types'

interface DetailProps {
  prompt: SavedPromptRecord; collections: PromptCollectionRecord[]; onBack: () => void; onCopy: () => void; onRemix: () => void; onProduction: () => void; onDuplicate: () => void; onExport: (structured: boolean) => void; onNotes: (notes: string) => void; onDelete: () => void; onAddToCollection: (id: string) => void
}

export function SavedPromptDetail({ prompt, collections, onBack, onCopy, onRemix, onProduction, onDuplicate, onExport, onNotes, onDelete, onAddToCollection }: DetailProps) {
  ...
  return (
    <div className="saved-detail">
      ...
      <section className="panel final-prompt workspace-prompt-detail">
        ...
        <div className="detail-actions"><button className="gold-button" onClick={onCopy}><Copy size={16} /> Copy Prompt</button><button className="outline-button" onClick={onRemix}><RefreshCcw size={16} /> Remix</button><button className="outline-button" onClick={onProduction}><Ruler size={16} /> Production Center</button><button className="outline-button" onClick={onDuplicate}><Files size={16} /> Duplicate</button><button className="outline-button" onClick={() => onExport(false)}><Download size={16} /> Export .txt</button><button className="outline-button" onClick={() => onExport(true)}><Download size={16} /> Full Details</button><button className="danger-button" onClick={onDelete}><Trash2 size={16} /> Delete</button></div>
      </section>
    </div>
  )
}
